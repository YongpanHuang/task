...
// 增加一个返回值，包含三个list的结构体,还需有一个函数对所有的vector进行合并
void DoTask(const std::vector<Row*>& rows, size_t start, size_t end, std::vector<std::list<Row*>> *list_vec) {
  int len = rows.size();
  // 二分查找剪枝
  ...
  // 以下三个list是天然有序的
  list_vec->push_back(BinarySearch(rows, start, end, 1000));
  list_vec->push_back(BinarySearch(rows, start, end, 2000));
  list_vec->push_back(BinarySearch(rows, start, end, 3000));
}
...
void Task3(const std::vector<Row*>& rows) {
  ...
  std::vector<std::vector<std::list<Row*>>> vec(thread_num + 1);
...
void MergeAll(std::vector<std::vector<std::list<Row*>>> *vec) {
